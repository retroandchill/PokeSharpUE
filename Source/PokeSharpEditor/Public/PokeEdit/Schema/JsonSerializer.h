...
namespace PokeEdit
{
    ...
    struct TJsonConverter<T>
    {
        static TValueOrError<T, FString> Deserialize(const TSharedRef<FJsonValue>& Value) requires TParsableEnum<T>
        {
            if (FString Result; Value->TryGetString(Result))
            {
        ...
        static TSharedRef<FJsonValue> Serialize(const T Value) requires TPrintableEnum<T>
        {
    ...
    {
        static TValueOrError<TArray<T>, FString> Deserialize(const TSharedRef<FJsonValue>& Value) requires TJsonDeserializable<T>
        {
            if (const TArray<TSharedPtr<FJsonValue>> *JsonValues; Value->TryGetArray(JsonValues))
            {
                ...
                for (const auto& JsonValue : *JsonValues)
                {
                    ...
                    if (FString Error; DeserializedValue.TryGetError(Error))
                    {
                        Errors.Add(Error);
                    }
                    else
                    ...
                    if (Errors.Num() > 0)
                    {
                        return MakeError(FString::Format(TEXT("Multiple errors were found when deserializing value\n {0}"), { FString::Join(Errors, TEXT("\n")) }));
                    }
                }
        ...
        static TSharedRef<FJsonValue> Serialize(const TArray<T>& Value) requires TJsonSerializable<T>
        {
    ...
    {
        static TValueOrError<TOptional<T>, FString> Deserialize(const TSharedRef<FJsonValue>& Value) requires TJsonDeserializable<T>
        {
            if (Value->IsNull())
            {
        ...
        static TSharedRef<FJsonValue> Serialize(const TOptional<T>& Value) requires TJsonSerializable<T>
        {
    ...
    TValueOrError<T, FString> Deserialize(const TSharedRef<FJsonValue>& Value)
    {
        return TJsonConverter<T>::Deserialize(Value);
    }
    
    template <TJsonSerializable T>
    TSharedRef<FJsonValue> Serialize(T&& Value)
    {
